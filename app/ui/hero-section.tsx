...
function CharacterShowCase() {
  return (
    <div className="absolute bottom-0 left-0 h-[60vh] lg:h-[30vh]">
      ...
      <div className="absolute bottom-0 flex h-full w-full items-center justify-between px-8 md:px-24 lg:px-40 xl:px-20">
        <CharacterList />
        <div className="relative h-[130%] w-1/2 overflow-visible md:w-1/3 lg:h-screen">
          <Image
            priority
            src={TheDJ}
            alt="character-bg"
            className="absolute bottom-0 left-0 h-full max-w-72 scale-x-[-1] object-cover md:-left-96 md:max-w-[50rem] lg:-left-[20rem] xl:-left-[12rem] xl:bottom-20 xl:max-w-[45rem] 2xl:-left-[8rem]"
          />
          <Image
            priority
            src={TheDJTitle}
    ...
    <>
      <div className="relative hidden lg:block xl:hidden">
        <div className="relative h-[10.5rem] w-[15.5rem] bg-[url('/characters/character-background.png')] bg-cover bg-center">
          ...
          />
          <span className="font-tekoSans absolute bottom-3 left-1/2 -translate-x-1/2 text-nowrap text-center font-semibold text-white">
            {characterInfo.title}
          </span>
        </div>
      </div>
      ...
      <div className="relative block lg:hidden xl:block">
        <div className="group relative z-10 h-20 w-36 cursor-pointer border-2 border-solid border-[#FBC625] bg-[url('/characters/character-background.png')] bg-cover bg-center md:h-24 md:w-40 lg:h-32 lg:w-48 xl:h-24 xl:w-40 2xl:h-[7.5rem] 2xl:w-[12.5rem]">
          ...
          />
          <span className="font-tekoSans text-md absolute -bottom-1 left-0 right-0 text-center font-semibold text-white md:text-xl xl:hidden">
            {characterInfo.title}
          </span>
        </div>
        <div className="absolute -left-2 top-2 h-20 w-36 bg-black md:h-24 md:w-40 xl:h-24 xl:w-40 2xl:h-[7.5rem] 2xl:w-[12.5rem]"></div>
        <span className="font-tekoSans xl:text-md absolute -bottom-8 left-0 right-0 mx-auto hidden text-nowrap text-center text-sm font-semibold xl:-bottom-10 xl:block">
          {characterInfo.title}
        </span>
      </div>
    </>
